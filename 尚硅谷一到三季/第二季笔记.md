...
```bash
D:\java exercise\framework-learn>java -XX:+PrintCommandLineFlags -version
-XX:InitialHeapSize=265667136 -XX:MaxHeapSize=4250674176 -XX:+PrintCommandLineFlags -XX:+UseCompressedClassPointers -XX:+UseCompressedOops -XX:-UseLargePagesIndividualAllocation -XX:+UseParallelGC
java version "1.8.0_251"
Java(TM) SE Runtime Environment (build 1.8.0_251-b08)
Java HotSpot(TM) 64-Bit Server VM (build 25.251-b08, mixed mode)
```

默认使用的是  -XX:+UseParallelGC



## 案例二十三：java7种垃圾回收器

![image-20210201201135052](https://gitee.com/chen_yi_fenga/blog-imag/raw/master/image-20210201201135052.png)

![image-20210201201503365](https://gitee.com/chen_yi_fenga/blog-imag/raw/master/image-20210201201503365.png)

### 1.串行GC （Serial）

会暂停所有用户线程，执行GC线程进行垃圾回收

对应JVM参数： -XX：+UseSerialGC

开启后： **Serial（Young区用**）+ **Serial Old(Old区用)**的收集器组合

表示：新生代、老年代会使用串行回收器，新生代使用复制算法，老年代使用标记-整理算法

### 2.并行GC （ParNew）

会暂停所有用户线程，执行**并行GC线程进行新生代**垃圾回收

对应JVM参数： -XX：+UseParNewGC 只影响新生代 不影响老年代

开启后： ParNew（Young区用）+ Serial Old(Old区用)的收集器组合

表示：新生代、老年代会使用串行回收器，新生代使用复制算法，老年代使用标记-整理算法

### 3.并行回收GC （Parallel）

ParPallel 是串行收集器在新生代和老年代的**并行化**

即新生代和老年代都实行并行线程进行垃圾回收

对应JVM参数： -XX：+UseParallelGC （java8默认）

### 4.并发回收器 （CMS）

Concurrent Mark Swreep 并发标记清除 并发指的是与用户线程一起执行

对应JVM参数: -XX：+UseConcMarkSweepGC 开启后自动将 -XX：+UseParNewGC 打开

开启后： 使用ParNew（Young区用**）+ **CMS(Old区用)的收集器组合。Serial Old将作为CMS出错的后备收集器
...
G1收集器的设计目标是取代CMS收集器。它与CMS有以下特点：

1. 不会产生很多内存碎片。整体采用 **标记-整理算法**。局部是通过**复制算法**
2. 在停顿时间上添加了预测机制。用户可以指定期望停顿时间
3. 宏观上不再分年轻代和老年代。把内存划分为多个独立的子区域（Region）。可以理解为一个围棋的棋盘
4. 虽然在整个内存区域混合在一起，但是本身依然在小范围进行年轻代和老年代的区分。保留了新生代和老年代，但是它们不再是物理隔离，而是一部分Region的集合且不需要Region是连续的。也就是说依然会采用不同的GC方法来处理不同的区域
5. **G1是由逻辑上的分代概念**。不需要完全独立survivor(to space)堆做复制准备。

**回收步骤：**

![image-20210201212736651](C:\Users\Cheny\AppData\Roaming\Typora\typora-user-images\image-20210201212736651.png)

![image-20210201212751890](https://gitee.com/chen_yi_fenga/blog-imag/raw/master/image-20210201212751890.png)

![image-20210201212819016](https://gitee.com/chen_yi_fenga/blog-imag/raw/master/image-20210201212819016.png)

## 案例二十四：linux常用命令

### cpu

**pidstat -u l -p {pid}**   查看线程占用cpu详细情况
...
iostat -xdk 2 3 每2秒取一次 共取3次

![image-20210201220544454](https://gitee.com/chen_yi_fenga/blog-imag/raw/master/image-20210201220544454.png)

![image-20210201220456766](https://gitee.com/chen_yi_fenga/blog-imag/raw/master/image-20210201220456766.png)
